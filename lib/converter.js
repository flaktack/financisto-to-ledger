...
function convertTransferTransaction(transaction, options) {
    let items = [
        {
...
function convertSplitExpenseTransaction(transaction, entities) {
    ...
    items.push(... transaction.splits.map(function (splitTransaction) {
        let noteValue = splitTransaction.note && splitTransaction.note.match(/^\(\((.*?)\s*(-?\d+\.?\d*)\s*(.*?)\)\)$/);
        let item = {};
        ...
        // Note-specified original_currency
            ...
            let amount = parseFloat(noteValue[2]),
                symbol = noteValue[1] || noteValue[3],
                currency = entities['currency'].find(c => c && c.symbol == symbol);
            item.cost = formatCurrency(currency, -amount * FIXED_INT);
            item.postingCost = formatCurrency(splitTransaction.from_account.currency, splitTransaction.from_amount);
        }
...
function convertTransaction(entities, transaction) {
    ...
    const date = transaction.datetime.format('YYYY/MM/DD HH:mm'),
    ...
    if (options.project && +transaction.project._id) {
        text += `\t; Project: ${transaction.project.title}\n`;
    }

    if (options.location && transaction.location && +transaction.location._id) {
        text += `\t; Location: ${transaction.location.name}\n`;
    }

    if (options.lonlat && transaction.longitude && transaction.latitude && transaction.provider) {
    ...
    if (transaction.from_account && transaction.to_account) {
        items.push(... convertTransferTransaction(transaction, options));
    }
